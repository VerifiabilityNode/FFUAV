...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(sprayInIN,PumpCommand::Off())).then(E(waterStatusOutOUT,WaterStatus::Ok())).then(E(waterStatusOutOUT,WaterStatus::Low()));
	...
	checker = checker.initially(test.getMessages()).then(E(sprayInIN,PumpCommand::Off())).pass(E(waterStatusOutOUT,WaterStatus::Ok())).forbid(E(waterStatusOutOUT,WaterStatus::Low()));
