...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(waterStatusOutOUT,WaterStatus::Ok())).then(E(sprayInIN,PumpCommand::On())).then(E(sprayInIN,PumpCommand::Off())).then(E(waterStatusOutOUT,WaterStatus::Low())).then(E(sprayInIN,PumpCommand::Off()));
	...
	checker = checker.initially(test.getMessages()).then(E(waterStatusOutOUT,WaterStatus::Ok())).then(E(sprayInIN,PumpCommand::On())).then(E(sprayInIN,PumpCommand::Off())).pass(E(waterStatusOutOUT,WaterStatus::Low())).forbid(E(sprayInIN,PumpCommand::Off()));
