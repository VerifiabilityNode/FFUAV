...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(sprayInIN,PumpCommand::Off())).then(E(sprayInIN,PumpCommand::On())).then(E(waterStatusOutOUT,WaterStatus::Low())).then(E(waterStatusOutOUT,WaterStatus::Low())).then(E(sprayInIN,PumpCommand::Off()));
	...
	checker = checker.initially(test.getMessages()).then(E(sprayInIN,PumpCommand::Off())).then(E(sprayInIN,PumpCommand::On())).then(E(waterStatusOutOUT,WaterStatus::Low())).pass(E(waterStatusOutOUT,WaterStatus::Low())).forbid(E(sprayInIN,PumpCommand::Off()));
