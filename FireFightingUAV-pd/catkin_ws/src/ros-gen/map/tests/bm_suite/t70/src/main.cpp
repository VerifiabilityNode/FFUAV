...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(batteryStateIN,Battery{1})).then(E(batteryStatusOUT,BatteryStatus::SafetyCritical())).then(E(batteryStatusOUT,BatteryStatus::Ok()));
	...
	checker = checker.initially(test.getMessages()).then(E(batteryStateIN,Battery{1})).pass(E(batteryStatusOUT,BatteryStatus::SafetyCritical())).forbid(E(batteryStatusOUT,BatteryStatus::Ok()));
