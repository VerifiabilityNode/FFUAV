...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,false)).then(E(fcMissionWPUpload,std::tuple<std::vector<Position>>{{1,3}}));
	...
	checker = checker.initially(test.getMessages()).then(E(fcActivation,std::tuple<>{})).pass(E(fcActivationRetIN,false)).forbid(E(fcMissionWPUpload,std::tuple<std::vector<Position>>{{1,3}}));
