...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).then(E(fcDroneTaskControl,std::tuple<M600Task::M600Task>{M600Task::GoHome()}));
	...
	checker = checker.initially(test.getMessages()).then(E(fcActivation,std::tuple<>{})).pass(E(fcActivationRetIN,true)).forbid(E(fcDroneTaskControl,std::tuple<M600Task::M600Task>{M600Task::GoHome()}));
