...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).then(E(fcControlAuth,std::tuple<>{})).then(E(fcControlAuthRetIN,true)).then(E(batteryStateInIN,Battery{3})).then(E(fcMissionWPAction,std::tuple<M600Action::M600Action>{M600Action::Resume()}));
	...
	checker = checker.initially(test.getMessages()).then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).then(E(fcControlAuth,std::tuple<>{})).then(E(fcControlAuthRetIN,true)).pass(E(batteryStateInIN,Battery{3})).forbid(E(fcMissionWPAction,std::tuple<M600Action::M600Action>{M600Action::Resume()}));
