...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(waterStatusOutOUT,WaterStatus::Ok())).then(E(sprayInIN,PumpCommand::Off())).then(E(sprayInIN,PumpCommand::On())).then(E(waterStatusOutOUT,WaterStatus::Low())).then(E(sprayInIN,PumpCommand::On()));
	...
	checker = checker.initially(test.getMessages()).then(E(waterStatusOutOUT,WaterStatus::Ok())).then(E(sprayInIN,PumpCommand::Off())).then(E(sprayInIN,PumpCommand::On())).pass(E(waterStatusOutOUT,WaterStatus::Low())).forbid(E(sprayInIN,PumpCommand::On()));
