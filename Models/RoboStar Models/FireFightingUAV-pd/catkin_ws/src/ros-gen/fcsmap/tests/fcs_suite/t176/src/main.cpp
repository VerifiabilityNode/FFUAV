...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcDroneTaskControl,std::tuple<M600Task::M600Task>{M600Task::Land()}));
	...
	checker = checker.initially(test.getMessages()).pass(E(fcActivation,std::tuple<>{})).forbid(E(fcDroneTaskControl,std::tuple<M600Task::M600Task>{M600Task::Land()}));
