...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).then(E(fcControlAuth,std::tuple<>{})).then(E(fcMissionWPAction,std::tuple<M600Action::M600Action>{M600Action::Pause()}));
	...
	checker = checker.initially(test.getMessages()).then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).pass(E(fcControlAuth,std::tuple<>{})).forbid(E(fcMissionWPAction,std::tuple<M600Action::M600Action>{M600Action::Pause()}));
