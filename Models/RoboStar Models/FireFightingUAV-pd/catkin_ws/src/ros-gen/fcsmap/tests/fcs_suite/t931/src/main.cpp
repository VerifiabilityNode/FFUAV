...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).then(E(fcControlAuth,std::tuple<>{})).then(E(fcMissionWPUpload,std::tuple<std::vector<Position>>{{3,2,0}}));
	...
	checker = checker.initially(test.getMessages()).then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).pass(E(fcControlAuth,std::tuple<>{})).forbid(E(fcMissionWPUpload,std::tuple<std::vector<Position>>{{3,2,0}}));
