...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcMissionWPUpload,std::tuple<std::vector<Position>>{{3,3}}));
	...
	checker = checker.initially(test.getMessages()).pass(E(fcActivation,std::tuple<>{})).forbid(E(fcMissionWPUpload,std::tuple<std::vector<Position>>{{3,3}}));
