...
int main(int argc, char** argv) {
	...
	auto trace = builder.then(E(fcActivation,std::tuple<>{})).then(E(fcActivationRetIN,true)).then(E(fcMissionWPAction,std::tuple<M600Action::M600Action>{M600Action::Pause()}));
	...
	checker = checker.initially(test.getMessages()).then(E(fcActivation,std::tuple<>{})).pass(E(fcActivationRetIN,true)).forbid(E(fcMissionWPAction,std::tuple<M600Action::M600Action>{M600Action::Pause()}));
