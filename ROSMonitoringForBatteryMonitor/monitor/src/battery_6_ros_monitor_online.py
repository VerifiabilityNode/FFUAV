...
def callbackbattery_status(data):
	global ws, ws_lock
...
def callbackinput_accepted(data):
	global ws, ws_lock
	rospy.loginfo('monitor has observed: ' + str(data))
	dict = message_converter.convert_ros_message_to_dictionary(data)
...
def monitor():
	...
	with open(log, 'w') as log_file:
		log_file.write('')
	rospy.init_node('battery_6_ros_monitor_online', anonymous=True)
	pub_error = rospy.Publisher(name = 'battery_6_ros_monitor_online/monitor_error', data_class = MonitorError, latch = True, queue_size = 1000)
	pub_verdict = rospy.Publisher(name = 'battery_6_ros_monitor_online/monitor_verdict', data_class = String, latch = True, queue_size = 1000)
	rospy.Subscriber(battery_status_topic, BatteryStatus, callbackbattery_status)
	rospy.Subscriber(input_accepted_topic, InputAccepted, callbackinput_accepted)
...
def main(argv):
	...
	log = '/home/robotlab/Practice/battery_6/log_online.txt' 
	actions = {
		battery_status_topic : ('log', 1), 
		input_accepted_topic : ('log', 1)
	}
	monitor()
